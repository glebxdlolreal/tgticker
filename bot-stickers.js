...
var NewPack = {
  ...
  submit() {
    var stickers = $('.tm-grid-item[data-doc]').toArray().map(el => ({doc_id: $(el).data('doc'), emoji: $(el).data('emoji') || 'рџ«Ґ'}));
...
var EditPack = {
  ...
  wrapEmojiHtml: function(text, lg) {
    ...
    return text.replace(/рџЏ»|рџЏј|рџЏЅ|рџЏѕ|рџЏї/g, '').replace(Aj.state.emojiRE, function(emoji) {
